...
def evaluate_SFT(peft_model, dataset, tokenizer, label_name, batch_size=16):
    ...
    for i, batch in enumerate(data_loader):
        ...
        for idx in range(len(outputs_text)):
            ...
            selected_sentiment = outputs_text[idx].split("User label: ")[1].strip()
            selected_sentiment = selected_sentiment.split("[/INST] ")[1]
            selected_sentiment = selected_sentiment.split("\n")[0].lower().strip()

            # print("Selected sentiment:", selected_sentiment)
            # assert 1==2

            if selected_sentiment not in label_name:
...
def main():
    ...
    parser.add_argument("--adapters_name", type=str, default="Llama-2-13b-chat-hf-epoch10-system-prompt")
    parser.add_argument("--dataset_name", type=str, default="OneFly7/llama2-politosphere-fine-tuning-system-prompt-without-definition")
    parser.add_argument("--label_name", type=str, default="['support gun polarized', 'support gun non-polarized', 'anti gun non-polarized', 'anti gun polarized', 'not relevant', 'not sure', 'neutral']")
    ...
    label_name = eval(args.label_name)
    print(label_name)
    ...
    predictions, invalid_label, golden_label = evaluate_SFT(peft_model, train_split, tokenizer, label_name, batch_size)
    print("Evaluation on the training dataset:")
    showEvalResults(predictions, invalid_label, golden_label)

    predictions, invalid_label, golden_label = evaluate_SFT(peft_model, validation_split, tokenizer, label_name, batch_size)
